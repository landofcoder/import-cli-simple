...
class UrlKeyUtil implements UrlKeyUtilInterface
{
     ...
     * @param StoreWebsiteRepository $storeWebsiteRepository
    ...
    protected function doMakeUnique(UrlKeyAwareSubjectInterface $subject, array $entity, string $urlKey, string $urlPath = null) : string
    {
        ...
        // $storeWebsite = $this->storeWebsiteRepository->findAll();
        ...
        do {
            ...
            $existedAllUrl = true;
            foreach ($checkStoreIds as $sid) {
                $check = $this->loadUrlRewriteByRequestPathAndStoreId($requestPath, $sid);
                if ($check) {
                    $urlRewrite = $this->loadUrlRewriteByRequestPathAndStoreId($requestPath, $sid);
                }
                if (is_array($check) && $check['entity_id'] != $entityId ) {
                    $existedAllUrl = false;
                }
            }
            ...
            if ($urlRewrite) {
                ...
                if (((int) $urlRewrite[MemberNames::ENTITY_ID]   === $entityId) &&
                           $urlRewrite[MemberNames::ENTITY_TYPE] === $entityType && ( $existedAllUrl
                           || !is_int($urlRewrite['url_rewrite_id']))
                ) {
